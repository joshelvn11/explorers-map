...
- A third checked-in read-only GPT-facing schema lives at `apps/web/openapi/explorers-map-actions.read-only.openapi.json`, and the runtime serves it from `/api/actions/openapi.read-only.json`.
